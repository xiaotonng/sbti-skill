...
26 种人格类型。每种包含：code（代码）、cn（中文名）、intro（一句话介绍）、desc（完整描述）、pattern（15维匹配模式，维度顺序：S1-S2-S3-E1-E2-E3-A1-A2-A3-Ac1-Ac2-Ac3-So1-So2-So3）。
...
## HHHH — 傻乐者（兜底类型）
- **Pattern**: 无固定 pattern（当最高匹配度 < 60% 时强制分配）
- **Intro**: 哈哈哈哈哈哈。
